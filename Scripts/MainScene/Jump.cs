...
public class Jump : MonoBehaviour
{
    ...
    public void OnTriggerEnter2D(Collider2D collider)
    {
        if (collider.CompareTag("Player"))
        {
            ...
            float distanceA = Vector3.Distance(luna.transform.position, targetA.position);
            float distanceB = Vector3.Distance(luna.transform.position, targetB.position);
            target = distanceA > distanceB ? targetA : targetB;
